...
class UserManager extends AbstractManager {
    ...
    insert(user) {
        return this.connection
            .query(`insert into ${this.table} (firstname, lastname, email, password, roles) VALUES (?, ?, ?, ?, ?)`,
                [user.firstname, user.lastname, user.email, user.password, JSON.stringify(user.roles)]
            )
            .then(([rows]) => {
